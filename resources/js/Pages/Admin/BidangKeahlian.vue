...
// Ambil data-data awal dari backend buat ditampilin
const props = defineProps({
    header: { type: Object, default: () => ({}) },
    skills: { type: Array, default: () => [] }
});

// Buat ngatur notif pop-up sukses atau error di pojok kanan atas
const showToast = ref(false);
const toastMessage = ref('');
const toastType = ref('success'); 

// Fungsi buat nampilin notifnya selama 3.5 detik
...
// Data buat ngatur tampilan kotak pop-up konfirmasi di tengah layar
const modal = ref({
    isOpen: false, title: '', message: '', icon: 'bx-question-mark', color: '#4E9BE0', confirmText: 'Ya', showCancel: true, onConfirm: null
});

// Kumpulan fungsi buat buka, tutup, sama eksekusi aksi di modal
...
// Daftar ikon yang bisa dipilih, murni pake bawaan Boxicons
const iconList = [
...
// Data bawaan kalau ternyata dari database belum ada isinya sama sekali
const defaultSkills = [
...
// Fungsi pinter buat benerin nama ikon lama jadi ikon Boxicons yang baru otomatis
const autoMigrateIcon = (oldIcon, skillName) => {
    const name = (skillName || '').toLowerCase();
    const icon = (oldIcon || '').toLowerCase();

    // Deteksi dari namanya dulu biar lebih akurat
    ...
    // Kalau gagal dari nama, baru deh cek dari nama ikon lamanya
    ...
    // Kalau ikonnya udah bener pake Boxicons, ya biarin aja
    const isAlreadyValid = iconList.some(i => i.val === oldIcon);
    if (isAlreadyValid) return oldIcon;

    // Kalau mentok gak nemu, kasih ikon default aja
    return 'bx bx-code-alt';
};

// Ambil data keahlian dari props terus langsung lewatin fungsi pinter di atas
...
// Setup form buat disiapin dikirim ke backend
const form = useForm({
...
// Ngatur timer buat bar peringatan kalau ada editan belum disave (ilang 5 detik)
...
// Fungsi-fungsi buat ngatur centang-centang massal
...
// Pengaturan paginasi (biar yang nampil dibatesin 5 per halaman)
...
// Fungsi nambah baris baru atau hapus baris keahlian
const addSkillRow = () => {
...
// Hapus semua yang lagi dicentang
...
// Buat batalin semua editan balik ke awal
const cancelForm = () => {
...
<template>
  ...
  <div class="admin-container">
    <div class="wrap-form">
      ...
      <form @submit.prevent="submit">
        ...
        <!-- Kotak besar buat ngatur list skillnya -->
        <div class="premium-card" style="margin-top: 40px;">
          ...
          <div class="card-body">
            ...
            <!-- Panel indikator jumlah skill sama tombol centang -->
            ...
            <!-- Panel menu aksi kalo lagi mode centang -->
            ...
            <!-- Tampilan kalau list keahlian bener-bener kosong -->
            ...
            <!-- Daftar tabel keahlian yang udah ke-load -->
            <div v-else class="slide-viewport">
              ...
              <div id="skills-container" class="skills-list">
                <div v-for="item in paginatedSkills" :key="item.idx" class="skill-row-item" :class="{'row-selected': selectedIndices.includes(item.idx), 'is-selectable': isSelectMode}" @click="handleRowClick(item.idx, $event)">
                  ...
                  <!-- Preview logo kecil di paling kiri -->
                  ...
                  <!-- Tombol tong sampah buat per baris -->
              ...
              <!-- Navigasi slide kalau datanya lebih dari 5 item -->
            ...
            <!-- Tombol gede buat nambah slot baris baru di bawah -->
  ...
  <!-- Bar kuning muncul dari atas ingetin nyimpen -->
  <Teleport to="body">
  ...
  <!-- Bagian pop up modal dan kotak toast -->
  <Teleport to="body">
...
<style scoped>
/* Ini bagian yang kurapihin bersusun ke bawah */
.admin-container {
  --bg: #0A0E17;
   --panel: #10151F;
    --panel-2: #141B29;
...
/* Nah ini kubikin sebaris amburadul */
.wrap-form { max-width: 1050px; margin: 0 auto; padding-bottom: 60px; }
...
.input-group input, .input-group textarea, .input-group select { width: 100%; background: var(--bg); border: 1px solid var(--line); color: #fff; padding: 14px 18px; border-radius: 12px; font-size: 14px; transition: 0.3s; box-sizing: border-box; font-family: 'Inter', sans-serif; }
.input-group input:focus, .input-group textarea:focus, .input-group select:focus { outline: none; border-color: var(--cyan); background: var(--panel-2); box-shadow: 0 0 0 4px rgba(78, 155, 224, 0.1); }
...
.btn-save { padding: 14px 32px; background: var(--primary); color: #fff; border: none; border-radius: 12px; font-weight: 600; cursor: pointer; transition: 0.3s; display: inline-flex; align-items: center; gap: 8px; box-shadow: 0 4px 15px rgba(55, 99, 224, 0.2); font-size: 14px; }
...
/* Campuran numpuk */
.btn-add-dashed:hover { border-color: var(--cyan); color: var(--cyan); background: rgba(78, 155, 224, 0.05); }
.select-mode-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px; padding-bottom: 16px; border-bottom: 1px dashed var(--line); }
.skill-count { font-size: 11px; font-family: 'JetBrains Mono', monospace; color: var(--dim); font-weight: 700; letter-spacing: 1px; }
.skill-count-page { color: var(--cyan); }
.btn-toggle-select { background: rgba(255,255,255,0.03); border: 1px solid var(--line); color: var(--text); padding: 8px 16px; border-radius: 8px; font-size: 12px; font-weight: 600; cursor: pointer; display: flex; align-items: center; gap: 6px; transition: 0.3s; }
...
/* Numpuk panjang */
.left-bulk, .right-bulk { display: flex; align-items: center; gap: 16px; }
.custom-checkbox { display: flex; align-items: center; gap: 10px; cursor: pointer; font-size: 13.5px; color: var(--text); font-weight: 600; user-select: none; }
...
/* Numpuk */
.skills-list-header .col-spacer { width: 38px; flex-shrink: 0; }
.skills-list-header span:nth-child(2) { flex: 1.6; }
.skills-list-header span:nth-child(3) { flex: 1.3; }
.skills-list-header span:nth-child(4) { width: 130px; flex-shrink: 0; }
@media (max-width: 760px) { .skills-list-header { display: none; } }
...
/* Numpuk habis */
.skill-row-item:hover { background: rgba(255,255,255,0.03); border-color: var(--line); }
.skill-row-item.row-selected { border-color: var(--cyan); background: rgba(78, 155, 224, 0.06); }
.skill-row-item.is-selectable { cursor: pointer; }
.skill-row-item.is-selectable:hover { border-color: var(--cyan); }
.icon-preview-sm { width: 38px; height: 38px; border-radius: 10px; display: flex; align-items: center; justify-content: center; font-size: 17px; flex-shrink: 0; }
.skill-input-name { flex: 1.6; min-width: 140px; background: var(--bg); border: 1px solid var(--line); color: #fff; padding: 10px 14px; border-radius: 10px; font-size: 13.5px; font-family: 'Inter', sans-serif; box-sizing: border-box; transition: 0.2s; }
.skill-input-name:focus { outline: none; border-color: var(--cyan); box-shadow: 0 0 0 3px rgba(78, 155, 224, 0.1); }
.skill-select-wrap { position: relative; flex: 1.3; min-width: 130px; }
.skill-select-wrap select { width: 100%; appearance: none; cursor: pointer; background: var(--bg); border: 1px solid var(--line); color: #fff; padding: 10px 32px 10px 14px; border-radius: 10px; font-size: 13.5px; font-family: 'Inter', sans-serif; box-sizing: border-box; transition: 0.2s; }
.skill-select-wrap select:focus { outline: none; border-color: var(--cyan); }
.custom-arrow-sm { position: absolute; right: 12px; top: 50%; transform: translateY(-50%); pointer-events: none; color: var(--dim); font-size: 11px; }
.skill-color-wrap { position: relative; width: 130px; height: 38px; background: var(--bg); border: 1px solid var(--line); border-radius: 10px; display: flex; align-items: center; gap: 10px; padding: 0 12px; cursor: pointer; box-sizing: border-box; flex-shrink: 0; transition: 0.2s; }
.skill-color-wrap:hover { border-color: var(--cyan); }
.color-dot-sm { width: 16px; height: 16px; border-radius: 5px; border: 1px solid rgba(255,255,255,0.2); flex-shrink: 0; }
.color-hex-sm { font-family: 'JetBrains Mono', monospace; font-size: 11.5px; color: var(--dim); font-weight: 600; }
.hidden-color-picker { position: absolute; inset: 0; opacity: 0; width: 100%; height: 100%; cursor: pointer; }
.btn-remove-skill-sm { background: transparent; color: var(--danger); border: 1px solid rgba(255, 59, 48, 0.2); width: 36px; height: 36px; border-radius: 10px; cursor: pointer; transition: 0.2s; display: flex; align-items: center; justify-content: center; font-size: 15px; flex-shrink: 0; margin-left: auto; }
.btn-remove-skill-sm:hover { background: var(--danger); color: #fff; border-color: var(--danger); }
@media (max-width: 760px) { .btn-remove-skill-sm { margin-left: 0; } }
...
/* Numpuk */
.btn-slide-nav { background: rgba(255,255,255,0.03); border: 1px solid var(--line); color: var(--text); padding: 10px 18px; border-radius: 10px; font-size: 13px; font-weight: 600; cursor: pointer; display: flex; align-items: center; gap: 6px; transition: 0.3s; }
.btn-slide-nav:hover:not(:disabled) { background: rgba(78, 155, 224, 0.1); border-color: var(--cyan); color: var(--cyan); }
.btn-slide-nav:disabled { opacity: 0.35; cursor: not-allowed; }
...
<style>
/* CSS global buat warning dan modal. Ini aku kasih format berjenjang rapi */
.unsaved-warning-bar {
  position: fixed;
   top: 30px;
    left: 50%;
...
/* Nah mulai sini aku hantam jadi sebaris-sebaris */
